...
<body>
    ...
    ?>
    <br>
    <form action="../ctrl/ctrl_circulo.php?id=<?php echo $id; ?>" method="post">
        Raio: <input type="text" name="raio" value="<?php if($acao == "editar") echo $vetor[0]["raio"]; ?>"><br>
        <br>
    ...
    <form method="post">
        ...
        <input type="radio" name="tipo" value="2" <?php if($tipo == 2) echo "checked"; ?>> Raio <br>
    ...
    <table border="1">
        ...
        <?php
            $lista = listaCirculo($tipo, $info);
            foreach($lista as $linha){
        ?>
        <tr>
            <th><?php echo $linha["idcirculo"]; ?></th>
            <td><?php echo $linha["raio"]; ?></td>
            <td><?php echo $linha["cor"]; ?></td>
            <td><?php echo $linha["tabuleiro_idtabuleiro"]; ?></td>
            <td><a href="show.php?obj=cir&id=<?php echo $linha["idcirculo"]; ?>">Visualizar círculo</a></td>
            <td><a href="circulo.php?acao=editar&id=<?php echo $linha["idcirculo"]; ?>">Editar</a></td>
            <td><a href="javascript:excluirRegistro('../ctrl/ctrl_circulo.php?acao=excluir&id=<?php echo $linha["idcirculo"]; ?>')">Excluir</a></td>
        </tr>
        <?php
            }
        ?>
